...
Pre-loaded credential intelligence for security exercises and engagements. EXPLOIT-001 reads this file at session start and when generating attack plans. Operators should review and supplement this file before starting operations.
...
These passwords work across nearly all security exercise events regardless of theme or year. Spray these first against every target.
...
### ⚠ Pattern A Carry-Over Priority Spray

**IMPORTANT: This section assumes the current environment is the quals base image carry-over. This is UNVERIFIED until the operator confirms topology match via pre-engagement scan or live recon. Read the confidence tiers before acting on any entry.**
...
- **UNKNOWN** — Structure is known but values are engagement-specific and unverifiable pre-engagement

**Verification gate:** Run pre-engagement NMAP during access window. If `.14` answers on 389/445/5985, `.20` answers on 80 (WordPress), and `.2` shows `bc:24:11:` MAC prefix → treat HIGH entries as operationally valid for T=0 spray. If topology does not match Pattern A, fall back to Universal Exercise Defaults below and run standard recon in Phase 1.

| Confidence | Target | Username | Password | Service | Notes |
|------------|--------|----------|----------|---------|-------|
| CONFIRMED | every group `.20` | admin | WaterIsWet?? | WordPress `/wordpress/wp-login.php` | Scoring engine verifies this — defensive teams CANNOT change it without losing SLA. Valid T=0 through end of engagement. |
| HIGH | every group `.22` | Administrator | WaterIsWet?? | WinRM/5985 | Quals base image default; defensive teams change this early |
| HIGH | every group `.14` | Administrator | WaterIsWet?? | SMB/445, LDAP/389 | Same image assumption; highest-value target if valid |
| HIGH | every group `.2` | root | WaterIsWet?? | SSH/22 | Spray this first — fastest initial access path |
| HIGH | every group `.10`,`.11` | root | WaterIsWet?? | SSH/22 | Secondary servers; same image |
| HIGH | every group `.240` | root | WaterIsWet?? | SSH/22 | Management host; often forgotten by defensive teams |
| ASSUMED | every group `.14` | rmwpra\Administrator | WaterIsWet?? | WinRM/5985 | Domain admin via WinRM; format: `DOMAIN\user` |
| ASSUMED | every group services | admin | WaterIsWet?? | Web apps, APIs | Theme password sprayed against all web admin interfaces |
| ASSUMED | every group `.14` | (see AD section) | WaterIsWet?? variants | LDAP/Kerberos | AD user spray; format FIRSTNAME_LASTNAME |

**Spray order for T=0 (Pattern A optimized):**
...
5. SSH `.240` with `root:WaterIsWet??` — management host; often overlooked by defensive teams

**If Pattern A carry-over is NOT confirmed:** skip this section, use Universal Exercise Defaults above, and run standard Phase 1 recon before spray.
...
### Pattern A (Historical Qualifiers — Hydration Theme)
...
### Pattern F (Historical — Star Wars Theme)
...
- **Defensive team reset templates (spray at T+5):** rainbowandhearts23012[username], [Word]-[Word]-[Word]-Dajda213

### Pattern E (Historical — Dairy/Farm Theme)
...
### Pattern C (Historical — MindMend / Mental Health Theme)
...
### Pattern D (Historical — Automotive Theme)
...
### Pattern B (Historical — Keycloak / Dinosaur Theme)
...
Cross-exercise patterns observed from historical PCAP analyses (Patterns A-F):

- Each exercise typically uses a thematic password (WaterIsWet??, FixTheBrain123!, etc.) except Pattern F which used per-account unique passwords
- General pattern: [ThemeWord][Verb/Adjective][Digits][Special] or [ThemeVerb][ThemeNoun][Digits][Special]
- Defensive team reset patterns tend to be formulaic: rainbowandhearts23012[user], [Word]-[Word]-[Word]-Dajda213
- Username formats vary by exercise: FIRSTNAME_LASTNAME (Pattern A), first-initial+lastname (Pattern B/E), character names (Pattern F)
